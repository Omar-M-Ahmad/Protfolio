/* ═══════════════════════════════════════════
   THEME — Dark / Light / System Toggle
═══════════════════════════════════════════ */

const STORAGE_KEY = 'oa-theme';
const THEMES = ['system', 'dark', 'light'];

const ICONS = {
  dark: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/>
  </svg>`,
  light: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <circle cx="12" cy="12" r="5"/>
    <line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/>
    <line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/>
    <line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/>
    <line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/>
  </svg>`,
  system: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <rect x="2" y="3" width="20" height="14" rx="2"/>
    <line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/>
  </svg>`,
};

const TITLES = {
  en: { dark: 'Dark Mode',  light: 'Light Mode', system: 'System' },
  ar: { dark: 'وضع داكن',  light: 'وضع فاتح',   system: 'النظام' },
};

let currentTheme = localStorage.getItem(STORAGE_KEY) || 'system';
let mediaQuery   = window.matchMedia('(prefers-color-scheme: dark)');

function resolveTheme(pref) {
  if (pref === 'system') return mediaQuery.matches ? 'dark' : 'light';
  return pref;
}

function applyTheme(pref, lang = 'en') {
  const resolved = resolveTheme(pref);
  document.documentElement.setAttribute('data-theme', resolved);
  document.documentElement.setAttribute('data-theme-pref', pref);

  const btn = document.getElementById('themeToggle');
  if (!btn) return;
  btn.innerHTML = ICONS[pref];
  btn.title = TITLES[lang]?.[pref] ?? TITLES.en[pref];
}

function cycleTheme(lang = 'en') {
  const idx = THEMES.indexOf(currentTheme);
  currentTheme = THEMES[(idx + 1) % THEMES.length];
  localStorage.setItem(STORAGE_KEY, currentTheme);
  applyTheme(currentTheme, lang);
}

function initTheme(lang = 'en') {
  applyTheme(currentTheme, lang);

  // Follow system preference changes when in "system" mode
  mediaQuery.addEventListener('change', () => {
    if (currentTheme === 'system') applyTheme('system', lang);
  });

  document.getElementById('themeToggle')?.addEventListener('click', () => {
    const lang = document.documentElement.lang || 'en';
    cycleTheme(lang);
  });
}

export { initTheme, applyTheme, TITLES };
